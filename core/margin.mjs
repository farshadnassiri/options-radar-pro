 ...
 * ——— مبنای جزء B ———
 *
 * صورتحساب واقعی کارگزاری برای دو استرانگل فروش، فقط با B×S تا ریال آخر
 * بازتولید شد. `SPOT` مبنای عملیاتی موتور است. پشتیبانی از `STRIKE` فقط در
 * تابع سطح‌پایین برای حسابرسی و مقایسهٔ متن‌های منتشرشده باقی مانده است.
 */
  ...
  return {
    ...
    note: (!isCredit
      ? (total > 0
        ...
        : 'بدهکار خالص — وجه تضمین گرفته نمی‌شود')
      : mode === 'FULL'
        ? 'بستانکار خالص — مبنای الف'
        : 'بستانکار خالص — مبنای انتخابی تو، تأییدنشده با تابلو')
      + (comboRule === 'MAX_PLUS_PREMIUM' ? ' · قاعدهٔ ترکیبی استرادل/استرانگل فروش' : '')
      + (p.bBasis === 'SPOT' ? ' · جزء B بر قیمت پایانی پایه' : ' · جزء B بر قیمت اعمال — فقط حالت حسابرسی'),
  };
}
