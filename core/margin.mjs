 ...
 * «بی‌تعهد». برای اسپرد ساده درست است — پای فروش با پای خرید پوشیده است و
 * کارگزار چیزی بلوکه نمی‌کند، پس هر سه عدد یکی می‌شوند و مخرج عوض نمی‌شود.
 * برای نسبت‌اسپرد نه: دو پا فروخته‌ای و یکی خریده، پس بخشی از فروش برهنه
 * می‌ماند. یک نمونه واقعی از اسکن ۳۱ استراتژی، بدهکار خالصِ ۲٬۰۰۰ ریال با
 * وجه تضمین ۱۹٬۰۰۴٬۵۰۰ و بیشترین زیانِ ۹۰٬۱۴۷٬۰۰۰ داشت. مخرج ۲٬۰۰۰ ریالی،
 * بازده ماهانهٔ ۲۲۹٬۷۶۸٪ می‌ساخت و آن ردیف صدر جدول می‌نشست — در حالی که
 * با ۲٬۰۰۰ ریال اصلاً نمی‌شود بازش کرد.
 *
...
export function capitalBase({ legs, netCash, marginNet, maxLoss }) {
  ...
  if (netCash < -EPS) {
    ...
    // شرط، خودِ وجه تضمین است نه بزرگیِ زیان. وجه تضمین دقیقاً وقتی مثبت
    // می‌شود که فروشی برهنه مانده باشد؛ اسپرد بدهکارِ پوشیده صفر می‌گیرد و
    // از این شاخه رد می‌شود، پس عددش مو‌به‌مو همان می‌ماند که بود.
    if (blocked > 0) {
      // همان استدلال سمت بستانکار: اگر وجه تضمین از بیشترین زیان کمتر
      // باشد، مابه‌التفاوت را خودت باید کنار بگذاری وگرنه سر سررسید
      // نقدینگی نداری. زیان نامحدود عدد نمی‌سازد و بیرون می‌ماند.
      const value = Math.max(debit, blocked, Number.isFinite(maxLoss) ? maxLoss : 0);
      return {
        value,
        kind: 'DEBIT_BLOCKED',
        label: Number.isFinite(maxLoss) && maxLoss >= blocked
          ? 'بدهکار خالص — بیشترین زیان ممکن'
          : 'بدهکار خالص + وجه تضمین بلوکه‌شده',
      };
    }
    return { value: debit, kind: 'DEBIT', label: 'بدهکار خالص پرداختی' };
  }
