...
export const SCHEMA = [
  ...
  // ═══ چرا پاسخِ خالی کشِ جدا دارد ═══
  //
  // ممیزی (۱۴۰۵/۰۶/۲۴): سه روزی که واقعاً معامله داشتند «نماد پایه معامله
  // نشد» گرفتند، و با سرورِ تازه همان درخواست‌ها دادهٔ کامل برگرداندند.
  // یعنی پاسخِ خالیِ لحظه‌ای، پانزده دقیقه در کش می‌ماند و مثل واقعیتِ
  // بازار رفتار می‌کند.
  //
  // فهرستِ خالی از یک پاسخِ عادی **بسیار** بیشتر احتمال دارد لغزشِ گذرای
  // بالادست باشد. پس عمرش کوتاه است: روزِ واقعاً بی‌معامله حداکثر یک
  // درخواست در دقیقه هزینه دارد، و لغزشِ گذرا خودش خوب می‌شود.
  { key: 'ttlEmptySec', group: 'data', kind: 'num', scope: 'server',
    def: 60, min: 0, max: 3600, step: 10, unit: 'ثانیه', label: 'عمر کش پاسخ خالی' },

  // ——— سهمیه و فشار روی سرور بازار ———
  // ═══ R5-08: چرا این سه عدد پایین آمدند ═══
  //
  // یک خروجیِ **یک‌هفته‌ایِ** تک‌پایه ۱٬۵۱۰ درخواست در ۱۵۶ ثانیه فرستاد —
  // حدود ۱۰ در ثانیه — و بالادست وسطِ کار سهمیه را بست. شکلِ بستنش
  // `HTTP 200` با آرایهٔ **خالی** است، پس ۳۵۹ ابزار/روز در فایل
  // «ریزمعامله نیامد» گرفتند در حالی که داده روی بورس بود و چند دقیقه
  // بعد با درخواستِ تکی کامل آمد.
  //
  // آزمونِ مستقل هم همین را داد: ۳۰۰ درخواست سالم، دورِ دوم نیمه‌خالی،
  // دورِ سوم **هر ۳۰۰تا خالی در پنج ثانیه**.
  //
