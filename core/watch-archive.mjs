...
/**
 * تابلوی زنده در دسترس نبود و تازه‌ترین روزِ بایگانی جایش نشست.
 *
 * این حالت با «بایگانی همان روز» یکی نیست و نباید مثل آن دیده شود: فهرست
 * مالِ روز دیگری است. بی‌برچسب ماندنش یعنی کاربر عکسِ روزِ دیگری را به‌جای
 * روزِ خواسته‌اش می‌بیند؛ پس روزِ واقعیِ فهرست همیشه در جمله می‌آید.
 */
export function archiveBoardDownNote({ fallbackDate, count = 0, wanted = 0 } = {}) {
  const fa = (n) => String(n).replace(/\d/g, (d) => '۰۱۲۳۴۵۶۷۸۹'[+d]);
  const head = `تابلوی زنده در دسترس نبود؛ فهرست از بایگانی ${fa(fallbackDate)} آمد — ${fa(count)} ردیف.`;
  return wanted && Number(wanted) !== Number(fallbackDate)
    ? `${head} این روزِ خواسته‌شده (${fa(wanted)}) نیست، پس سوگیری بقا دارد و عدد جلسه دقیق نیست.`
    : `${head} فهرست زنده نیست و ممکن است با امروز فرق داشته باشد.`;
}
