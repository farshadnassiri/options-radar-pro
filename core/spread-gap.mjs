...
export function measureGap({ legs = [], prices = {}, strategyId = '', entry = NaN, daysLeft = NaN } = {}) {
  ...
  //
  // بی این خط، همان ردیف «۰٪ پر شده، ۱۰۰٪ جا دارد» می‌شد و با مرتب‌سازی
  // بر «بیشترین جای باقی‌مانده» **صدرِ جدول** می‌نشست — بهترین پیشنهادِ
  // برنامه، ساختاری که وجود ندارد. در اجرای آزمایشی با دادهٔ ساختگی
  // دقیقاً همین رخ داد.
  if (!(current > 0)) {
    return { ...base, why: 'ارزش خالص این ساختار صفر است؛ دو پا هم‌قیمت‌اند و فاصله‌ای برای سنجیدن نمی‌ماند' };
  }
