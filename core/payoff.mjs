...
export function positionGreeks(legs, greeksByLeg) {
  ...
  // جمعِ ناقص، جمع نیست.
  //
  // حلقه از صفر شروع می‌کند و پایی که یونانی ندارد را رد می‌کند. یعنی
  // موقعیتی که هیچ پایش تلاطم نداده «دلتا ۰» می‌گرفت — عددی که دقیقاً
  // شبیه «این موقعیت خنثای جهت است» خوانده می‌شود، در حالی که حرفش
  // «نمی‌دانیم» است. پرچم `incomplete` کنارش بود ولی چشم اول به عدد
  // می‌افتد نه به پرچم.
  //
  // یونانی پاهایی که درآمده‌اند سر جایشان می‌مانند و در جدول‌های تفکیک
  // دیده می‌شوند؛ چیزی که ساخته نمی‌شود فقط ادعای «یونانی کل موقعیت این
  // است» است. `evaluate` هم در مرحله یک غربال همین را برمی‌گرداند، پس این
  // قرارداد تازه نیست — فقط دیگر استثنا ندارد.
  if (anyBad) for (const key of ['delta', 'gamma', 'vega', 'theta', 'rho', 'deltaShares']) out[key] = NaN;
  return out;
}
