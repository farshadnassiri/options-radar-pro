...
//
// آن پنل، زیرمنوی شناورِ سرگروه‌های ریل بود. بستنش پس از انتخاب، نیمهٔ راه
// بود؛ راهِ کامل این شد که اصلاً پنل شناوری نماند: هر ردیفِ ریل خودش دکمهٔ
// تب است و هیچ چیزی روی محتوا نمی‌نشیند. پس این‌بار ادعا **نبودن** است.
const appSrc = readSrc('../ui/app.mjs');
check('هیچ زیرمنوی شناوری روی محتوا نمی‌نشیند',
  !appSrc.includes('rail-submenu') && !appSrc.includes('closeSubmenu')
  && !readSrc('../ui/index.html').includes('rail-submenu'));
check('و انتخابِ تب مستقیم است، بی واسطهٔ پنل',
  /b\.addEventListener\('click', \(\) => \{ open\(t\.id\); \}\);/.test(appSrc));
