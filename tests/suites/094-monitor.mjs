...
group('۹۳. لایهٔ مشترک رصد — یک ورودی برای هر سه تایم‌فریم');
{
  ...
  // تبِ «رصد یونانی و تلاطم» به خواستهٔ صاحب پروژه از فهرست کناری برداشته
  // شد. ماژولش روی دیسک مانده ولی هیچ راهی از رابط به آن نمی‌رسد، پس
  // ادعای «ثبت شده» دیگر درست نیست — جایش همین را می‌سنجیم.
  check('تب رصد یونانی دیگر در فهرست تب‌ها ثبت نیست',
