...
group('۲۸۷. تب، قفل و هشدار را جدا نشان می‌دهد');
{
  ...
  // R5-18: با فهرستِ پس از بازیابیِ تاریخِ عرضه، نه فهرستِ انتخابِ خام.
  check('کم‌داشته‌ها به فایل می‌رسند', tab.includes('coverageWarnings: exportWarnings(universe, instruments),'));
