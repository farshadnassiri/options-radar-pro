...
group('۲۵۲-الف. لایهٔ توکن دولایه است');
{
  ...
  // ── پلهٔ برند در دو پوسته یکی نیست، ۱۴۰۵/۰۶/۲۸ ──
  //
  // ادعای پیشین این بود که هر دو پوسته پلهٔ ۶۰۰ را پر کنند «تا دکمهٔ اصلی
  // در دو پوسته یک رنگ باشد». وقتی زمینهٔ پوستهٔ تیره به #0a0b0d رسید، آن
  // رنگ روی زمینه ۳٫۱:۱ کنتراست داد و دکمهٔ اصلی خاموش دیده شد — همان
  // «بیسیک» بودنی که صاحب پروژه گزارش کرد. پلهٔ ۴۰۰ روی همان زمینه ۹٫۱:۱
  // می‌دهد.
  //
  // چیزی که این دسته از اول محافظت می‌کرد **دولایگی** بود نه عددِ پله:
  // نقش نباید مستقیم به هگز بسته شود. پس ادعا همان را می‌سنجد، و فقط
  // اجازه می‌دهد پله در دو پوسته فرق کند.
  check('نقشِ --accent در هر دو پوسته به پلهٔ خامِ برند بسته شده نه به هگز',
    /--accent:\s*var\(--b-\d{3}\)/.test(light) && /--accent:\s*var\(--b-\d{3}\)/.test(dark));
  check('و پوستهٔ تیره پلهٔ روشن‌تری از برند را پر می‌کند',
    /--accent:\s*var\(--b-600\)/.test(light) && /--accent:\s*var\(--b-400\)/.test(dark));
