...
group('۲۸۶. R4-03 — جدولِ پیوستهٔ سالم نباید رد شود');
{
  ...
  // ═══ چرا متنِ انتظار عوض شد (R5-03) ═══
  //
  // ادعا ضعیف‌تر نشد، **دقیق‌تر** شد: پیش از این ابزار فقط جمعِ کلِ برگ
  // را با `تعداد روزهای دیده‌شده × سطل` می‌سنجید، و همان بود که حذفِ یک
  // روزِ کامل را نمی‌دید (چون هر دو طرفِ تساوی با هم کم می‌شدند). حالا
  // سنجش روزبه‌روز و در برابرِ **روزهای برگ پوشش** است، پس پیام هم
  // روزبه‌روز حرف می‌زند.
  check('و عددش را می‌گوید',
    shortOut.out.includes('1 روز از 1 سطلِ کامل ندارد')
      && shortOut.out.includes('120 در برابر 180'),
    shortOut.out.split('\n').filter((l) => l.includes('سطل')).join(' | '));
}
