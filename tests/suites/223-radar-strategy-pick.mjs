// ۲۲۳. انتخاب استراتژی به نام خودش، نه به نام خانواده‌اش
//
// ═══ خواستهٔ صاحب پروژه ═══
//
...
group('۲۲۳. کشویی استراتژی در رادار فاصله');
...
check('صافیِ ساخت روی شناسهٔ استراتژی می‌افتد، نه روی گروهش',
  src.includes("def.id === family") && !src.includes('def.group === family'));
check('برچسب کنترل و گزینهٔ «همه» هم دیگر نام خانواده را نمی‌برند',
  src.includes('<label>استراتژی<select id="gr-family">')
