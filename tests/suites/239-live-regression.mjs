...
check('و پیوستن به درخواستِ در پرواز، عجله را با خودش می‌برد',
  serverSrc.includes('if (held) { boostTicket(held.ticket, priority); upLog(dl, { cat: \'joined\', path: pathname, url }); return held.promise; }')
  && serverSrc.includes('if (joined) { boostTicket(joined.ticket, priority); upLog(dl, { cat: \'joined\', path: pathname, url: `${S.baseUrl}${pathname}`, fresh: true }); return joined.promise; }'));
check('تلاشِ دوم هم اولویتِ تازه را می‌گیرد، نه اولویتِ صدازنندهٔ اول',
  serverSrc.includes('schedule(() => fetchUpstream(url, meta), ticket.priority, ticket, pathname)'));
