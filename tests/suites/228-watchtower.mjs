...
group('۲۲۸-الف. تب در برنامه ثبت است و جدا از رادار');
{
  ...
  // کاربر این تب را با واژه‌های خودش صدا می‌زند — «هشدار»، «اعلان»، «شرط».
  // جعبهٔ جست‌وجوی ریل برداشته شد، پس همان واژه‌ها روی عنوان راهنمای ردیف
  // می‌نشینند، جایی که در ریلِ جمع‌شده تنها معرفِ آیکون است.
  check('با واژه‌های خودِ کاربر معرفی می‌شود، نه فقط با نامش',
    /id: 'watchtower'[\s\S]{0,200}alias: '[^']*هشدار[^']*اعلان[^']*شرط/.test(app));
  // «رادار فاصله» به خواستهٔ صاحب پروژه حذف شد؛ دیده‌بان ماند و مستقل است.
  check('و مستقل ماند، نه زیرتبی در رادارِ حذف‌شده',
