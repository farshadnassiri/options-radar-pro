// ۲۲۵. نام نماد — در جدول، در کشویی، در نمودار، و در اعلان
//
// ═══ خواستهٔ صاحب پروژه ═══
//
...
group('۲۲۵. نام نماد در جدول، کشویی، نمودار و اعلان');
...
// جدول «اکنون» از قرارداد ستونیِ مشترک ساخته می‌شود، پس ستون در فهرست
// ستون‌ها اعلام می‌شود نه در `<th>` دست‌نویس — و در نمای پیش‌فرض هم هست،
// وگرنه ستونی که باید دیده شود پشتِ پنلِ ستون‌ها پنهان می‌ماند.
const cols = readSrc('../ui/radar-columns.mjs');
check('جدول «اکنون» ستون نماد دارد و آن را از پاهای همان ردیف می‌سازد',
  /key: 'symbols', label: 'نمادها'/.test(cols)
  && cols.includes('symbolCell(row.__row?.legs || [])')
  && /const DEFAULT_KEYS = \[[\s\S]*?'symbols'/.test(cols));
...
// یک تابع، دو مصرف‌کننده: ستون جدول و شناسنامه. دو نسخهٔ جدا یعنی روزی
// نماد در یکی درست نوشته می‌شود و در دیگری نه.
check('هر دو از یک تابع می‌آیند، نه از دو نسخهٔ جدا',
  /export function symbolCell/.test(cols)
  && /import \{[^}]*symbolCell[^}]*\} from '\/ui\/radar-columns\.mjs'/.test(src));
...
check('نام نماد دست‌نخورده نوشته می‌شود، نه با رقم فارسی‌شده',
  !/faDigits\((?:esc\()?leg\.name/.test(cols) && cols.includes('esc(leg.name)'));
