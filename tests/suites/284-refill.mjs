...
group('۲۸۴. سقفِ تلاش، و حافظهٔ بین دورها');
{
  ...
  //
  // ادعای قبلی «۵ ثانیه تا ۲ دقیقه» بود، بر این فرض که ناپایداریِ
  // بالادست گذراست. آزمونِ واقعی فرض را رد کرد: سهمیهٔ بسته‌شده دست‌کم
  // نیم‌ساعت دوام دارد، پس هر چهار دور داخلِ همان پنجره می‌افتادند و
  // همه خالی برمی‌گشتند. فاصله ضعیف‌تر نشد — **واقعی‌تر** شد.
  check('دورِ اول نیم‌دقیقه مکث می‌کند', refillDelayMs(1) === 30000);
  check('و هر دور دو برابر می‌شود', refillDelayMs(2) === 60000 && refillDelayMs(3) === 120000);
  check('ولی از یک ربع بالاتر نمی‌رود', refillDelayMs(20) === 900000);
  check('دورِ نامعتبر هم فاصلهٔ معتبر می‌دهد', refillDelayMs(0) === 30000);
}
