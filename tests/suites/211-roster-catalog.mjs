...
group('۲۱۱-ج۲. قرارداد بی‌معاملهٔ تابلوی جاری بذر کاتالوگ می‌شود');
{
  ...
  // این ردیف عمداً یک اختیار خرید **تبعی** است. تا امروز این دسته فقط
  // می‌پرسید «از دفتر نیفتاده؟» و سمتش را نمی‌سنجید — پس کالِ استانداردِ
  // جعلی از هفت‌هزار آزمونِ سبز رد می‌شد. قاعده‌اش در دستهٔ ۲۶۹ است.
  check('و سمتش تبعی است، نه کالِ استاندارد', got.rows[0].side === 'tabaee', got.rows[0].side);
