...
group('۹۴. یونانی و تلاطم در تب تحلیل تاریخی');
{
  ...
  // مقصدِ «رصد یونانی» حذف شد، پس دکمه‌اش هم رفت: دکمه‌ای که به جایی نرسد
  // بدتر از نبودِ دکمه است. یونانی‌های همین مسیر سرِ جایشان در همین تب‌اند.
  check('دکمهٔ رفتن به تب حذف‌شدهٔ رصد یونانی نمانده',
    !src94.includes("'greeks-watch'") && src94.includes('id="h-greeks-kpis"'));
