...
group('۴۷. نوار سقف سررسید، وقتی زنجیره نیست');
{
  ...
  check('`history/universe` وقتی عکس لحظه‌ای خالی است خودش از بالادست می‌گیرد',
    /history\/universe[\s\S]{0,320}watch\.rows\.length\s*\n?\s*\?\s*watch\.rows\s*\n?\s*:\s*firstList/.test(serverSrc47));
...
group('۷۱. موقعیت باز و تغییر آن');
{
  ...
  check('جدول عکس داشبورد هم ستون تغییر موقعیت باز گرفت',
    dash71.includes('<th>تغییر موقعیت باز</th>') && dash71.includes('fmt.int(row.oiChange)'));
}
