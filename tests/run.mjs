...
import {
  ...
  makeArchive, chainRowsFrom, archiveNote, archiveQuality,
} from '../core/watch-archive.mjs';
import {
  DATA_QUALITY_VERSION, DATA_QUALITY_KINDS, makeDataQuality, isDataQuality,
  combineDataQuality, dailyDataQuality, intradayDataQuality, bookDataQuality,
  universeDataQuality,
...
import {
  ...
  createPortfolioMission, portfolioMissionSummary, validateMissionOutlook,
  validateMissionRisk, validateMissionLiquidity, portfolioMissionRiskBudget,
} from '../core/portfolio-mission.mjs';
import {
  parseTomanInput, parsePercentInput, parseIntegerInput, tomanToRial,
  previewPortfolioAllocations, previewPortfolioCapital, previewPortfolioRisk,
  createPortfolioAllocationDraft, createPortfolioStepOneDraft,
...
group('۱۱۵. پوسته UI مأموریت — مرحله نخست');
{
  ...
  check('این برش مأموریت را فعال یا پیشنهاد تولید نمی‌کند',
    !tab.includes('activatePortfolioSession') && !tab.includes('createPortfolioMission')
...
group('۱۱۶. UI مأموریت — انتظار بازار');
{
  ...
  check('UI برای outlook فقط آداپتر قرارداد مشترک را مصرف می‌کند',
    tab.includes('createPortfolioOutlookDraft(setupDraft')
    && !tab.includes('createPortfolioMission') && !tab.includes('activatePortfolioSession')
    && !tab.includes('generateCandidates'));
...
group('۱۱۷. UI مأموریت — ریسک و نقدشوندگی');
{
  ...
  check('UI هنوز مأموریت یا پیشنهاد را فعال نمی‌کند',
    !tab.includes('createPortfolioMission') && !tab.includes('activatePortfolioSession')
...
group('۱۱۸. UI مأموریت — تخصیص خانواده‌ها');
{
  ...
  check('تخصیص دسکتاپ جدول و در موبایل ردیف تک‌ستونه دارد',
    ...
    && css.includes('.pt-allocation-totals { grid-template-columns: 1fr; }'));
}
