...
group('۳. رقم نمایشی فارسی است');
{
  ...
  // ═══ مرزِ نمایش، نه فقط خودِ قالب‌بند ═══
  //
  // ادعاهای بالا خودِ توابع را می‌سنجند و همیشه سبز بودند — ولی ایراد
  // جای دیگری بود: `historyDateLabel` در `core/` است و رقمِ **لاتین**
  // برمی‌گرداند (و باید هم برگرداند؛ خروجی CSV رقم فارسی نمی‌خواهد).
  // هر جا همان رشته مستقیم داخل یک رشتهٔ الگویی نشست، روی صفحه
  // «1405/12/15» ماند. عکس‌گرفتن از پانل جزئیاتِ تاریخی نشانش داد و
  // هفده مورد در سه تب پیدا شد.
  //
  // پس مرز اینجاست: داخلِ رشتهٔ الگویی یعنی نمایش، و نمایش یعنی باید از
  // `faDigits` رد شود. بیرونِ رشتهٔ الگویی (ستون CSV، کلید مقایسه) دست
  // نمی‌خورد.
  const rawDateUse = [];
  // `walk` پسوند را **اجباری** می‌خواهد؛ بی آن فهرست خالی برمی‌گردد و
  // ادعا بی‌آنکه چیزی بسنجد سبز می‌ماند — همان آزمونِ توخالی که این
  // مخزن دو بار گرفتارش شده.
  for (const file of walk(path.join(ROOT, 'ui'), '.mjs')) {
    const text = fs.readFileSync(file, 'utf8');
    const hits = (text.match(/\$\{historyDateLabel\(/g) || []).length;
    if (hits) rawDateUse.push(`${path.relative(ROOT, file)}: ${hits}`);
  }
  check('تاریخ نمایشی مستقیم داخل رشتهٔ الگویی نمی‌نشیند — از faDigits رد می‌شود',
    rawDateUse.length === 0, rawDateUse.join(' ،') || 'هیچ');
}
...
group('۱۰. چیدمان آزمون سبد کپی نمی‌شود');
{
  ...
  // ═══ ثانیهٔ جلسه از ساعتِ ماشین ساخته نمی‌شود ═══
  //
  // گزارش ۱۴۰۵/۰۶/۱۷: «ساعت سیستم ۰۳:۳۹، ساعت تهران ۱۳:۰۹ … برنامه سقف
  // زمانی را ۰۳:۳۹ تشخیص می‌دهد؛ یعنی قبل از شروع بازار.» نتیجه‌اش صفر
  // لحظه و «هر دو پا امروز معامله نشده» بود، در حالی که تابلو هزاران
  // ریزمعامله داشت.
  //
  // کلِ موتور روی «ثانیهٔ جلسهٔ تهران» کار می‌کند و `tehranSecondOfDay` از
  // قبل برای همین بود؛ فقط صدا زده نمی‌شد. این ایراد هیچ استثنایی
  // نمی‌اندازد و هیچ آزمونی را قرمز نمی‌کرد — چون آزمون‌ها `until` را عددِ
  // آماده می‌دادند.
  //
  // الگو دقیق است و کاربردِ مشروع ندارد: ساعتِ نمایشی H:M:S قالب می‌شود،
  // در ۳۶۰۰ ضرب نمی‌شود. هر جا کسی دوباره ثانیهٔ روز از ساعتِ محلی بسازد،
  // اینجا قرمز می‌شود.
  const localSecond = [];
  for (const file of [...walk(path.join(ROOT, 'ui'), '.mjs'), ...walk(path.join(ROOT, 'core'), '.mjs')]) {
    const text = fs.readFileSync(file, 'utf8');
    if (/getHours\(\)\s*\*\s*3600/.test(text)) localSecond.push(rel(file));
  }
  check('ثانیهٔ جلسه از ساعتِ محلیِ ماشین ساخته نمی‌شود — tehranSecondOfDay هست',
    localSecond.length === 0, localSecond.join(' ،') || 'هیچ‌کدام');
}
