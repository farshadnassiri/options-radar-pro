...
export function heatRamp(value, lo, hi, declared) {
  ...
  if (lo < 0 && hi > 0) {
    ...
    // ═══ در ستونِ «زیان»، مثبت خبرِ بد است ═══
    //
    // این شاخه وقتی می‌آید که ستون هر دو علامت را دارد، و تا امروز علامتِ
    // خام را می‌خواند: مثبت سبز، منفی قرمز. برای «بیشترین سود» درست است و
    // برای «بیشترین زیان» **کاملاً وارونه** — چون موتور زیان را اندازه
    // می‌نویسد (عدد مثبت).
    //
    // نتیجه‌اش روی جدولِ واقعی: ردیفی با «بیشترین زیان ٪ سرمایه ۵۰» سبزِ
    // پررنگ می‌شد و ردیفی با ‎−۸‎ — یعنی ترکیبی که **در هیچ قیمتی زیان
    // نمی‌دهد** — قرمزِ پررنگ. و این حالت خیال‌پردازی نیست: باکس و
    // کانورژنِ سودِ قطعی دقیقاً همان چیزی‌اند که این برنامه دنبالشان
    // می‌گردد، و کنارِ ردیف‌های عادی همین بازه را می‌سازند.
    //
    // همان اشتباهی که یک بار در پانل جزئیات گرفته شد — آنجا تفاضلِ خام
    // علامت می‌داد و کم‌شدنِ زیان قرمز درمی‌آمد — این بار در نقشهٔ رنگِ جدول.
    const good = declared === 'loss' ? value < 0 : value >= 0;
    return { tone: good ? 'gain' : 'loss', t: Math.sqrt(Math.min(1, Math.abs(value) / side)) };
  }
  const span = hi - lo;
 ...
 *
 *   cols     ستون‌های شروع  [{ key, label, fmt, heat, sign, group, pin }]
  ...
  function draw() {
    ...
    for (let i = first; i < last; i++) {
      ...
      for (const c of shown) {
        ...
        // ── ستونی که علامت دارد، هر دو علامت را رنگ می‌کند ────────────
        //
        // تا امروز فقط منفی‌ها رنگ می‌گرفتند: مثبت و «بدون تغییر» هر دو با
        // رنگ متن عادی نوشته می‌شدند و در یک ستون پر از عدد، رشد از سکون
        // جدا نمی‌شد. `sign` را همان ستونی اعلام می‌کند که علامتش خبر است
        // (تغییر قیمت، تغییر موقعیت باز، فاصله تا سربه‌سر) — نه هر ستون
        // عددی، وگرنه «حجم» هم سبز می‌شود و رنگ معنی‌اش را از دست می‌دهد.
        const isPos = c.sign && isNum && Number.isFinite(v) && v > 0;
        td.className = `${isNum ? 'n' : ''}${isNeg ? ' neg' : ''}${isPos ? ' pos' : ''}${c.cell ? ' rich' : ''}`;
        // ── سلولِ نگاره‌دار ──────────────────────────────────────────
        //
