...
/**
 * نقشهٔ بردنِ یک موقعیتِ ثبت‌شده به تب تحلیل رول.
 *
 * ═══ چرا فقط شناسه می‌رود، و نه خودِ موقعیت ═══
 *
 * تب رول موقعیت‌ها را از `/api/positions` می‌خواند — همان منبعی که تب
 * موقعیت‌ها از آن خوانده. فرستادنِ کپیِ رکورد یعنی دو نسخه از یک موقعیت در
 * دو صفحه، و اگر کاربر وسطش ویرایشی کرده باشد، مقصد رویِ نسخهٔ کهنه حساب
 * می‌کند. شناسه، همیشه به تازه‌ترین نسخه می‌رسد.
 *
 * `title` فقط برای پیامِ خطاست: اگر موقعیت در مقصد پیدا نشد (حذف شده،
 * بسته شده)، باید بشود گفت **کدام** موقعیت پیدا نشد.
 */
export function positionRollPlan(pos) {
  const id = String(pos?.id || '');
  if (!id || !(pos?.legs || []).some((leg) => leg.kind !== 'underlying')) return null;
  return {
    to: 'roll', from: 'positions',
    positionId: id,
    title: String(pos?.title || ''),
    uaIns: String(pos?.uaIns || ''), uaName: String(pos?.uaName || ''),
  };
}
...
// `ic` نامِ آیکون از همان خانوادهٔ `ui/icons.mjs` است، نه ایموجی. ایموجی
// رنگِ ثابتِ خودش را دارد: روی دکمهٔ ثانویه در پوستهٔ تیره با متن هم‌رنگ
// نمی‌شود، و در ستونِ عددی عرضِ خانه را می‌لرزاند. آیکونِ خانواده
// `currentColor` می‌گیرد و در هر دو پوسته درست است. نامِ آیکونِ هر مقصد
// همان نامِ آیکونِ خودِ تب در `TAB_ICON` است تا دکمه و ردیفِ ریل یک نشان
// داشته باشند.
export const STRATEGY_LINK_TARGETS = [
  { to: 'backtest', ic: 'play', label: 'آزمایشگاه آپشن',
    why: 'همین ترکیب را روی تاریخ بیازما — بازه، پاها و حجم از همین ردیف می‌روند.' },
  { to: 'watchtower', ic: 'bell', label: 'دیده‌بان شرطی',
    why: 'برای همین ترکیب قاعده بگذار؛ شرط با عددِ همین لحظه پیش‌پر می‌شود.' },
  { to: 'positions', ic: 'plus', label: 'افزودن به موقعیت‌های من',
