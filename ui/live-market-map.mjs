...
export function mountLiveMarketMap(root, { onScopeChange = null, contractColumns = [] } = {}) {
  ...
  function paintDailyRanges() {
    ...
    rangeChart.innerHTML = `<div class="lmm-range-legend"><span><i class="rail"></i>کمترین تا بیشترین</span><span><i class="body"></i>اولین تا آخرین</span><span><i class="close"></i>پایانی</span></div><div class="lmm-range-list">${rows.map((row) => {
      ...
      return `<article class="${tone(last - first)}"><header><button type="button" data-lmm-range-contract="${esc(row.ins)}"><b>${esc(row.name)}</b><small>${kindLabel(row.kind)} · اعمال ${fmt.money(row.strike)}</small></button><strong>${rankValue}</strong></header><div class="lmm-range-track" title="کمترین ${fmt.money(low)} · اولین ${fmt.money(first)} · آخرین ${fmt.money(last)} · پایانی ${fmt.money(close)} · بیشترین ${fmt.money(high)}"><i class="lmm-range-rail"></i><i class="lmm-range-body" style="--range-left:${left.toFixed(2)}%;--range-width:${width.toFixed(2)}%"></i><i class="lmm-range-first" style="--range-at:${firstAt.toFixed(2)}%"></i><i class="lmm-range-last" style="--range-at:${lastAt.toFixed(2)}%"></i><i class="lmm-range-close" style="--range-at:${closeAt.toFixed(2)}%"></i></div><footer><span>کمینه ${fmt.money(low)}</span><span>اولین ${fmt.money(first)}</span><span>آخرین ${fmt.money(last)}</span><span>پایانی ${fmt.money(close)}</span><span>بیشینه ${fmt.money(high)}</span></footer></article>`;
    }).join('')}</div>`;
