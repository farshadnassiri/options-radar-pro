...
export function mountLiveMarketMap(root, { onScopeChange = null, contractColumns = [] } = {}) {
  root.innerHTML = `
    ...
    <section class="card lmm-explorer" data-lmm-explorer>
      ...
      <section class="lmm-day-range" data-lmm-day-range hidden><div class="lmm-step-head"><div><h3>بازه قیمت امروز قراردادها</h3><span>خط: کمترین تا بیشترین · بدنه: اولین تا آخرین · لوزی: قیمت پایانی</span></div><div class="lmm-range-sort" data-lmm-range-sort role="group" aria-label="مرتب‌سازی نمودار بازه روزانه"></div></div><div data-lmm-range-status class="note"></div><div data-lmm-range-chart></div></section>
  ...
  function paintDailyRanges() {
    ...
    rangeChart.innerHTML = `<div class="lmm-range-legend"><span><i class="rail"></i>کمترین تا بیشترین</span><span><i class="body"></i>اولین تا آخرین</span><span><i class="last"></i>آخرین</span><span><i class="close"></i>پایانی</span><small>روی نمودار بزنید تا جزئیات باز بماند؛ نام قرارداد وارد جزئیات آن می‌شود.</small></div><div class="lmm-range-list">${rows.map((row) => {
      ...
      return `<article class="${tone(last - first)}" data-lmm-range-card="${esc(row.ins)}"><header><button type="button" data-lmm-range-contract="${esc(row.ins)}"><b>${esc(row.name)}</b><small>${kindLabel(row.kind)} · اعمال ${fmt.money(row.strike)}</small></button><strong>${rankValue}</strong></header><button type="button" class="lmm-range-track" data-lmm-range-focus="${esc(row.ins)}" aria-expanded="false" aria-label="بازه روزانه ${esc(row.name)}؛ تغییر از اولین تا آخرین ${fmt.pct(change)} درصد"><i class="lmm-range-rail"></i><i class="lmm-range-body" style="--range-left:${left.toFixed(2)}%;--range-width:${width.toFixed(2)}%"></i><i class="lmm-range-first" style="--range-at:${firstAt.toFixed(2)}%"></i><i class="lmm-range-last" style="--range-at:${lastAt.toFixed(2)}%"></i><i class="lmm-range-close" style="--range-at:${closeAt.toFixed(2)}%"></i><span class="lmm-range-popover"><b>${fmt.pct(change)}٪ از اولین تا آخرین</b><small>کمینه ${fmt.money(low)} · بیشینه ${fmt.money(high)}</small><small>اولین ${fmt.money(first)} · آخرین ${fmt.money(last)} · پایانی ${fmt.money(close)}</small></span></button><footer><span>کمینه ${fmt.money(low)}</span><span>اولین ${fmt.money(first)}</span><span>آخرین ${fmt.money(last)}</span><span>پایانی ${fmt.money(close)}</span><span>بیشینه ${fmt.money(high)}</span></footer></article>`;
    }).join('')}</div>`;
