...
/**
 * چرا این قرارداد وارد ترکیب‌سازی نشد.
 *
 * سه علت سه کارِ متفاوت می‌طلبند و در یک سطل ریختنشان کاربر را سردرگم
 * می‌کند: «بی‌سابقه» یعنی آن روز هنوز چیزی نبوده و کاری هم نمی‌شود کرد؛
 * «معامله نشد» یعنی بوده و بازار به آن دست نزده؛ «زیر دروازه» یعنی خودِ
 * کاربر آستانه گذاشته و می‌تواند پایینش بیاورد.
 */
const EXCLUDE_REASONS = {
  unseen: 'تا آن روز هیچ معامله‌ای نداشت',
  silent: 'آن روز معامله نشد',
  illiquid: 'زیر دروازهٔ نقدشوندگیِ خودت',
};
...
export function portfolioBacktestWorkbook(analysis, {
  ...
  const sourceRows = (sources || []).map((row) => [
    ...
    String(row.ins ?? ''), row.purpose ?? '', row.source ?? '', row.status ?? '',
    cell(row.rows),
  ...
  const excludedRows = (census?.excluded || []).map((row) => [
    row.name ?? '', row.kind === 'call' ? 'اختیار خرید' : 'اختیار فروش',
    cell(row.strike), dateLabel(row.expiry),
    EXCLUDE_REASONS[row.reason] || row.reason,
    row.firstTrade ? dateLabel(row.firstTrade) : '',
    String(row.ins ?? ''),
  ]);
  ...
  return [
    ...
    sheet('منبع داده', ['نقش', 'نماد', 'نوع', 'قیمت اعمال', 'سررسید', 'کد نماد', 'برای چه',
      ...
      sourceRows, [80, 140, 90, 100, 110, 160, 240, 300, 190, 80, 100, 100, 220, 480, 300]),
