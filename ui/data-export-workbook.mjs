...
export function buildDataExportSheets({
  ...
  const summary = sheet('راهنما', ['شاخص', 'مقدار'], [
    ...
    // ═══ R5-08: اگر سهمیه بسته شد، اولین چیزی است که باید دیده شود ═══
    //
    // «نیامد» و «نگذاشتند بیاید» دو چیزند، و دومی راهِ حلِ متفاوتی دارد:
    // صبر، نه تلاشِ دوباره. فایل باید همین را بگوید، وگرنه کاربر دورِ
    // تکمیلی می‌زند و سهمیه را بیشتر می‌سوزاند.
    ['سهمیهٔ بالادست', (() => {
      const hit = coverage.filter((row) => row.throttled).length;
      return hit
        ? `${hit} ابزار/روز پشتِ سهمیهٔ بالادست ماندند و دریافتشان متوقف شد.`
          + ' این‌ها «بی‌معامله» نیستند و «نیامد» هم نیستند — پرسیده نشدند.'
          + ' چند ساعت بعد همین بازه را دوباره بگیرید؛ آنچه آمده سرِ جایش می‌ماند.'
        : 'سهمیهٔ بالادست در این اجرا بسته نشد';
    })()],
