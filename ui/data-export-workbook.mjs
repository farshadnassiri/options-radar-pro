...
export function buildDataExportSheets({
  ...
  const summary = sheet('راهنما', ['شاخص', 'مقدار'], [
    ...
    ['مبنای تاریخ عرضه', basis.total
      ? `${basis.official} قرارداد از ${basis.total} تاریخ عرضهٔ رسمی دارند`
        ...
        + `${basis.fromDaily ? ` · برای ${basis.fromDaily} قرارداد آغازِ عمر از اولین روزِ معامله در تابلوی روزانهٔ خودش آمده — پیش از آن روز ریزمعامله‌ای نبود که جا بماند` : ''}`
    ...
    ['سهمیهٔ بالادست', (() => {
      ...
      const asked = coverage.filter((row) => row.throttled && row.asked).length;
      // ═══ R5-17: دو جمله، و راهِ درستِ ادامه ═══
      //
      // جملهٔ قبلی همه را «پرسیده نشدند» می‌نوشت و می‌گفت «همین بازه را
      // دوباره بگیرید؛ آنچه آمده سرِ جایش می‌ماند». هر دو نادرست بود:
      // بخشی پرسیده شده بودند، و خروجیِ تازه همه‌چیز را از صفر می‌گیرد.
      // راهی که داده را نگه می‌دارد دکمهٔ «تلاش تکمیلی» در همان صفحه است.
      return hit
        ? `${hit} ابزار/روز پشتِ سهمیهٔ بالادست ماندند`
          + `${asked ? ` — ${asked} تا پرسیده شدند و بالادست فهرستِ خالی داد (همین‌ها مدرکِ بسته‌شدنِ سهمیه‌اند)،`
            + ` ${hit - asked} تا اصلاً پرسیده نشدند` : ' و پرسیده نشدند'}.`
          + ' این‌ها «بی‌معامله» نیستند و «نیامد» هم نیستند.'
          + ' برای تکمیل، دست‌کم نیم ساعت بعد دکمهٔ «تلاش تکمیلی» را در همان صفحه بزنید:'
          + ' آنچه آمده سرِ جایش می‌ماند. ساختنِ خروجیِ تازه همه را از صفر می‌گیرد و سهمیه را دوباره می‌سوزاند.'
  ...
  const coverageSheet = sheet('پوشش دریافت', [
    ...
    return [
      ...
      // ═══ R5-17: هر پرچم چه داد، وقتی نوار کامل نشد ═══
      //
      // آزمونِ عملی ۱۳ ردیفِ «ناقص» داشت که همه «پرچم true» بودند، و فایل
      // نمی‌گفت چرا `true` انتخاب شد. جوابش در `tried` بود: `false` هیچ
      // نداده بود. این همان نشانه‌ای است که ناقص‌بودن را از «بالادست کم
      // داد» به «پرچمِ اصلی بسته بود» می‌برد — و درمانِ این دو فرق دارد.
      hit.variant
        ? `پرچم ${hit.variant}${Array.isArray(hit.tried) && hit.tried.length > 1
          ? ` (${hit.tried.map((t) => `${t.variant}: ${n(t.trades)} معامله`).join(' · ')})` : ''}`
        : '',
