...
// بودجهٔ صبر: بیست تلاش، جمعاً حدود ۱٫۸ ثانیه.
//
// نسخهٔ اول ده تلاش و ۳۲۳ میلی‌ثانیه بود و job ویندوز **دوباره** قرمز شد.
// حساب ساده است: خواننده‌ای که پیوسته می‌خواند، مقصد را بیشترِ وقت باز نگه
// می‌دارد؛ هر تلاش فقط وقتی موفق می‌شود که در فاصلهٔ بینِ دو خواندن بیفتد.
// با نیمی از زمان آزاد، ده تلاش یعنی یک درصد شکست به‌ازای هر تغییرِ نام —
// که در بیست‌وپنج نوشتن، شکست تقریباً حتمی است. بیست تلاش همان عدد را به
// یک در میلیون می‌رساند.
//
// کراندار می‌ماند و خطای واقعی را نمی‌پوشاند: مجوزِ نداشته بعد از همین
// بودجه با **همان خطای اصلی** بالا می‌رود، فقط دو ثانیه دیرتر. برای نوشتنِ
// پس‌زمینهٔ دفتر، دو ثانیه صبر ارزان‌تر از یک شکستِ کاذب است.
export const RETRY_DELAYS_MS = [
  1, 2, 4, 8, 16, 32, 50, 50, 75, 75,
