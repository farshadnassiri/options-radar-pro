...
// دوباره.
// ═══ R5-18: «سهمیه»ای که تابلو می‌گوید بی‌معامله بوده، سهمیه نیست ═══
//
// آزمونِ عملیِ پنج‌دوره ۲۲ ردیف داشت با وضعیتِ «سهمیه» و در **همان ردیف**
// ستونِ «حکم خالی‌بودن»: «بدون معامله — تابلوی روزانه هم صفر است». حکمِ
// تابلو حرفِ آخر است: نوارِ خالی برای روزی که معامله نداشته، کامل است.
// این تشخیص از ستونِ حکم است تا روی فایل‌های پیش از اصلاح هم درست بگوید.
const verdictAt = at('حکم خالی‌بودن');
const boardQuietRow = (r) => verdictAt >= 0 && String(r[verdictAt] ?? '').startsWith('بدون معامله');
const mislabeled = D.filter((r) => status(r).includes('سهمیهٔ بالادست') && boardQuietRow(r)).length;
const throttled = count('سهمیهٔ بالادست') - mislabeled;
// ═══ R5-12: روزِ جاری نقصِ دریافت نیست ═══
//
// تابلوی روزانهٔ روزِ جاری تا تسویه نهایی نمی‌شود، پس سنجیدنش ممکن
// نیست و نبودنِ حکم عیب نیست. شمردنش در مخرجِ پوشش یعنی هر خروجی که
// روزِ امروز را هم بگیرد، محکوم به «پوششِ ناکافی» باشد.
const openDay = count('روزِ جاری');
const partial = count('ناقص');
const quiet = count('بدون معامله') + mislabeled;
...
console.log(`  تطبیق‌شده ${matched} · نیامد ${missing} · ناقص ${partial} · بی‌معامله ${quiet}`
  + ` · تأییدنشده ${unverified} · روزِ جاری ${openDay} · سهمیه ${throttled} · خطا ${errored}`
  + `  (از ${total})`);
// ═══ R5-18: کاملِ ردیف‌ها با کاملِ فهرست یکی نیست ═══
//
// آزمونِ عملیِ پنج‌دوره: این ابزار روی فایلی حکمِ «دادهٔ کاملِ بازه» داد که
// سی قرارداد نداشت — کلِ سریِ ۶۰xx که ۱۴۰۵/۰۶/۲۵ سررسید شد، با ۶۹٬۳۳۶
// ریزمعامله. همهٔ بخش‌های پایین فقط ردیف‌هایی را می‌سنجند که **در فایل
// هستند**؛ قراردادی که هرگز کشف نشد، ردیفی ندارد که رد شود.
//
// خودِ فایل این را گفته بود — برگ راهنما: «پوشش دفتر قراردادها: ناقص»
// و «۵ روزِ کاریِ این بازه هنوز اسکن نشده». ابزار آن را نمی‌خواند.
const rosterCover = String(guide.get('پوشش دفتر قراردادها') ?? '');
const rosterGaps = String(guide.get('کم‌داشتهٔ دفتر هنگام خروجی') ?? '');
const unscanned = /اسکن نشده|کشف‌نشده/.test(rosterGaps);
say(!(rosterCover.startsWith('ناقص') || unscanned),
  'فهرستِ قراردادها کامل است — قراردادِ کشف‌نشده‌ای بیرون نمانده',
  rosterCover.startsWith('ناقص') || unscanned
    ? `برگ راهنما: «${rosterCover || '—'}»${rosterGaps ? ` · ${rosterGaps}` : ''}.`
      + ' ردیف‌هایی که در فایل هستند ممکن است کامل باشند، ولی قراردادی که کشف نشده در هیچ سنجشی نمی‌آید.'
      + ' پیش از «آماده‌سازی»، صبر کنید تا گام اولِ تبِ خروجی دیگر «پوشش ناقص» نگوید.'
    : '');
if (mislabeled) {
  say('warn', 'برچسبِ «سهمیه» روی ردیفِ بی‌معامله',
    `${mislabeled} ردیف «سهمیه» نوشته شده‌اند ولی تابلوی روزانه‌شان صفر است —`
    + ' این‌ها کامل‌اند و در «بی‌معامله» شمرده شدند؛ فایل پیش از اصلاحِ R5-18 ساخته شده');
}
say(missing === 0, 'هیچ ابزار/روزی «نیامد» نیست',
  missing ? `${missing} ابزار/روز تابلو معامله ثبت کرده ولی ریزمعامله نیامد` : '');
// ═══ R5-17: از ردیف‌های سهمیه، کدام پرسیده شدند ═══
//
// آزمونِ عملیِ ۱۴۰۵/۰۷/۰۱: این خط «۳۰۹ ابزار/روز … پرسیده نشدند» نوشت در
// حالی که ۲۵ تایشان با هر دو پرچم پرسیده شده و فهرستِ خالی گرفته بودند —
// یعنی همان مدرکِ بسته‌شدنِ سهمیه. تشخیص از ستونِ «پرچم درخواست» است، نه
// از متنِ وضعیت، تا روی فایل‌های پیش از این تفکیک هم درست بگوید.
const flagAt = at('پرچم درخواست');
const throttledAsked = D.filter((r) => status(r).includes('سهمیهٔ بالادست') && !boardQuietRow(r)
  && flagAt >= 0 && String(r[flagAt] ?? '').trim()).length;
say(throttled === 0, 'سهمیهٔ بالادست در این اجرا بسته نشد',
  throttled ? `${throttled} ابزار/روز پشتِ سهمیه ماندند`
    + `${throttledAsked ? ` — ${throttledAsked} تا پرسیده شدند و بالادست فهرستِ خالی داد، ${throttled - throttledAsked} تا اصلاً پرسیده نشدند` : ' و پرسیده نشدند'}.`
    + ' برای تکمیل، دست‌کم نیم ساعت بعد «تلاش تکمیلی» را در همان صفحه بزنید؛'
    + ' خروجیِ تازه همه را از صفر می‌گیرد و سهمیه را دوباره می‌سوزاند' : '');
...
// مخرج، روزِ جاری را کنار می‌گذارد: آن روز سنجیدنی نیست، پس نه در
// صورت می‌آید نه در مخرج.
const judged = Math.max(0, total - openDay);
const coverage = judged ? (matched + quiet) / judged : 0;
say(coverage >= 0.995, 'پوششِ تأییدشده دست‌کم ۹۹٫۵٪ است',
  `${(coverage * 100).toFixed(1)}٪ تأییدشده از ${judged} ابزار/روزِ سنجیدنی`
  + `${openDay ? ` (${openDay} ابزار/روزِ امروز کنار گذاشته شد — تابلویشان هنوز نهایی نشده)` : ''}`);
...
for (const r of D) {
  ...
  cell.all += 1;
  // ═══ R5-17: «دریافت‌نشده»، نه فقط «نیامد» ═══
  //
  // آزمونِ عملی سه روزِ کامل داشت که هر ۷۹ ردیفشان «سهمیه» بود، و این
  // بخش نوشت «✔ بلوکِ پیوستهٔ روزهای خراب وجود ندارد» — چون فقط «نیامد»
  // را می‌شمرد. روزی که هیچ‌چیزش گرفته نشده، بدترین روزِ ممکن است،
  // علتش هر چه باشد.
  const st = status(r);
  if (st.includes('ریزمعامله نیامد') || (st.includes('سهمیهٔ بالادست') && !boardQuietRow(r))
    || st.startsWith('خطا') || st.includes('درخواست نرفت')) cell.miss += 1;
  byDay.set(d, cell);
}
const days = [...byDay.entries()].sort((a, b) => a[0] - b[0]);
const badDays = days.filter(([, c]) => c.miss > 0);
say(badDays.length === 0, 'هیچ روزی شکاف ندارد',
  badDays.length ? `${badDays.length} روز از ${days.length} ردیفِ دریافت‌نشده دارند` : '');
...
if (badDays.length) {
  ...
  for (const [d, c] of [...badDays].sort((a, b) => b[1].miss - a[1].miss).slice(0, 5)) {
    console.log(`      ${faDate(d)}  ${c.miss}/${c.all} دریافت‌نشده`);
  }
}
...
if (tryAt < 0) {
  ...
  if (throttled) {
    // R5-17: این‌ها **لازم** دارند — فقط نه همین حالا. جملهٔ قبلی می‌گفت
    // «لازم ندارد»، که کاربر را از تنها راهِ تکمیلِ فایل دور می‌کرد.
    say('warn', 'ردیفِ پشتِ سهمیه پس از بازشدنِ سهمیه دوباره پرسیده شود',
      `${throttled} ابزار/روز — دست‌کم نیم ساعت بعد، با «تلاش تکمیلی» در همان صفحه؛`
      + ' تلاشِ فوری فقط سهمیه را بیشتر می‌بندد');
  }
}
